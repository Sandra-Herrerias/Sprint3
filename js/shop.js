...
function generateCart() {
  ...
  //Count duplicates. Number of duplicates it is stored as the key value
  ...
  //Add quantity attribute to each product
  cartList.forEach((prod) => {
    prod.quantity = 0;
  });
  console.log("cartQuantity" + JSON.stringify(cartList));

 cartList.forEach(function(i,j){

    if(counter[key].id == i+1){
myData.push({
 ...
 console.log("cartQuantity" + JSON.stringify(myData));
  ...
  //Only duplicates in array cart
  cart = cartList.filter((element, index) => {
    return cartList.indexOf(element) === index;
  });
  console.log("cartQuantity" + JSON.stringify(cart));
}
